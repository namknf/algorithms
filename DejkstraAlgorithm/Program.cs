namespace DejkstraAlgorithm
{
    ...
    using System.Linq;
    using static DejkstraAlgorithm.Graph_E;

    public class Program
    {
        public static List<Node> DejkstraAlgorithm(Graph graph, Dictionary<Edge, double> weights, Node start, Node end)
        {
            List<Node> notVisited = graph.ToList();
            ...
            while (true)
            {
                ...
                {
                    if (track.ContainsKey(v) && track[v].Price)
                    {
                        toOpen = v;
                ...
                foreach (var e in toOpen.IncidentEdges.Where(z => z._from = toOpen))
                {
